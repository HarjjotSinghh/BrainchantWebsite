...
const formSchema = z.object({
	...
	article_image_url: z.string().url('Image link must be a valid URL.').optional(),
	article_tags: z
...
export default function AddArticleForm({ userProfile, articleTags }: { userProfile: any; articleTags: any[] }) {
	...
	async function onSubmit(values: z.infer<typeof formSchema>) {
		try {
			...
			if (error) {
				...
				setMessage(
					`Successfully published your article at ${window.origin}/articles/${form
						.getValues()
			...
			// console.log(error)
			setMessage('Error publishing article. Please try again later or contact support.');
		} finally {
			setLoading(false);
		}
	}
		...
		<div className="lg:px-16 px-4 py-24 pb-48 flex justify-center items-center gap-16">
			<Form {...form}>
				<form
					...
					className="space-y-8 rounded-2xl lg:p-12 p-6 border-border/50 border-2 hover:border-border/70 transition-all duration-300 ease-in-out"
				>
					<Link href={'/dashboard'} className="flex flex-row gap-2 items-center self-end w-fit">
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel className="lg:text-lg text-base">Article Title</FormLabel>
								<FormControl>
									<Input placeholder="How To Do...." {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel className="lg:text-lg text-base">Article Image URL</FormLabel>
								<FormControl>
									<Input placeholder="A URL of the article image" {...field} />
								</FormControl>
								<FormMessage />
							</FormItem>
						)}
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel className="lg:text-lg text-base">Article Tags</FormLabel>
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								<FormLabel className="lg:text-lg text-base">Article Content</FormLabel>
