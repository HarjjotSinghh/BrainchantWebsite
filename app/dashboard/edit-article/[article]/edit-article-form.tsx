...
export default function EditArticleForm({ articleData, articleTags }: { articleData: any; articleTags: any[] }) {
	...
	async function onSubmit(values: z.infer<typeof formSchema>) {
		try {
			...
			// console.log(error)
			setMessage('Error updating the article. Please try again later or contact support.');
		} finally {
			setLoading(false);
		}
	}
		...
		<div className="lg:px-16 px-4 py-24 pb-48 flex flex-col justify-center items-center gap-16">
			<Form {...form}>
				<form
					...
					className="space-y-8 rounded-2xl lg:p-12 p-6 relative border-border/50 border-2 hover:border-border/70 transition-all duration-300 ease-in-out"
				>
