...
import { CiLocationOn, CiMoneyBill, CiCalendarDate, CiUser } from 'react-icons/ci';
import { IoIosAddCircleOutline, IoIosPerson } from 'react-icons/io';
import { Link } from 'next-view-transitions';
import { FaRegCalendarAlt, FaRegUser } from 'react-icons/fa';

function MainDashboardPage({ userProfile, articles }: { userProfile: any; articles: any[] }) {
	...
	async function handleArticleDeletion(e: any) {
		try {
			setLoading(true);
			const { error: error } = await supabase.from('articles').delete().eq('id', e.id);
	...
	return (
		<div className="lg:px-16 px-4 py-24 pb-48 flex lg:flex-row flex-col justify-center lg:items-start items-center gap-16">
			{/* <div className="flex lg:w-fit w-full lg:justify-center justify-center flex-col gap-4 lg:items-center items-center rounded-2xl lg:p-12 p-4 border-border/50 border-2 hover:border-border/70 transition-all duration-300 ease-in-out">
        ...
        <ul className="flex flex-col justify-start items-start gap-2 w-full">
          ...
          <li></li>
			...
			<div className="flex justify-center items-center flex-col lg:w-[650px] w-full gap-4 lg:p-12 p-4 rounded-2xl border-border/50 border-2 hover:border-border/70 transition-all duration-300 ease-in-out">
				<h1 className="lg:text-4xl text-3xl font-bold tracking-tight">
					<span className="text-foreground">Articles</span> Listed
				</h1>
				{articles.length === 0 && (
					<h1 className="lg:text-lg text-base">You do not have any articles published yet.</h1>
				)}
				{!articles
					? null
					: articles.map((e, i) => (
										...
										<AlertDialog>
											...
											<AlertDialogContent className="border-destructive">
												<AlertDialogHeader>
													...
													<AlertDialogDescription>
														Doing this will <i>permanently</i> delete your article{' '}
														<b> {e.article_title}</b>.
													</AlertDialogDescription>
												</AlertDialogHeader>
												<AlertDialogFooter>
