...
export const TracingBeam = ({ children, className }: { children: React.ReactNode; className?: string }) => {
	...
	return (
		<motion.div ref={ref} className={cn('relative max-w-[100dvw] w-full h-full', className)}>
			<div className="absolute left-4 md:-left-20 top-[-100%] z-[99]">
				<motion.div
					transition={{
						duration: 0.2,
						delay: 0.5
					...
					className="ml-[54px] h-4 w-4 rounded-full border border-netural-200 shadow-sm flex items-center justify-center"
				>
					<motion.div
						transition={{
							duration: 0.2,
